...
coords <- crds(records) # Identify duplicate coordinates
dup <- duplicated(coords)
sum(dup)  # how many exact duplicates found
records <- terra::subset(records, !dup) # Keep only unique coordinate records
nrow(records)

hosts <- rast("../dsdata/abares_modal_hosts.tif")                              # host data
```
...
- Identify uncategorised records outside of the CLIMEX mapping extent. Note and plot their coordinates for transparency. 
```{r rasterise}
...
crds(uncategorised)
plot(uncategorised, col = "red", pch = 19)
plot(world_boundary, add = T)
```
...
```{r girreo}
girreo <- rast("../dsdata/Assimila_IrrigatedAreas_candidate_EVI_idx_Global_2023_irrigation_mask_0.1666667Deg.tif")
...
```{r gbifrecs}
world_boundary_buffered <- terra::buffer(world_boundary, width = 5000)  # 5 km, adjust as needed
region_info <- terra::extract(world_boundary_buffered, records) # region_info has an 'ID' column linking back to record row number
region_info_unique <- region_info[!duplicated(region_info$id), ] # keep first match per record (in case of overlapping buffers)
...
nrow(gi_pts)
nrow(ng_pts)
sum( nrow(ei_pts) + nrow(gi_pts)+ nrow(ng_pts))
```
...
### Classify point type for plotting and tabulation
```{r}
...
```{r recordsmap}
type_col <- c("Record on EI" = "olivedrab", "Record on GI" = "steelblue1", "Record on GI <1 gen" = "#ad4387" )

png("../dsoutputs/recs_global.png", units="in", width=8, height=4, res=600)
par(mar = c(1, 1, 1, 1)) # plot 1
plot(world_boundary_robin, border = "black", lwd = 0.4,
     main = expression('Global records of '*italic(Drosophila~suzukii)),
     legend = FALSE, axes = FALSE, cex.main = 0.8)
plot(ei_pts_robin, col="olivedrab", cex=0.2, pch=20, legend=FALSE, add=TRUE)
plot(gi_pts_robin, col="steelblue1", cex=0.2, pch=20, legend=FALSE, add=TRUE)
plot(ng_pts_robin, col="#ad4387", cex=0.2, pch=20, legend=FALSE, add=TRUE)

legend("bottom", inset = c(0, -0.13), xpd = NA,
       legend = c(paste0("Record on EI (n=", nrow(ei_pts), ")"),
                  paste0("Record on GI (n=", nrow(gi_pts), ")"),
                  paste0("Record on GI <1 gen (n=", nrow(ng_pts), ")")),
       pch = 20, pt.cex = 1, bty = "n", cex = 0.5, horiz = FALSE,
       col = c("olivedrab", "steelblue1", "#ad4387"),
       title = "Record type")
knitr::include_graphics("../dsoutputs/recs_global.png")
```
...
```{r globalmaps}
png("../dsoutputs/D.suzukii_global.png", units="in", width=4, height=5, res=600)
...
knitr::include_graphics("../dsoutputs/D.suzukii_global.png")
```
...
```{r ausmaps}
png("../dsoutputs/D.suzukii_aus.png", units="in", width=10, height=5, res=700)
...
```{r nameuro}
png("../dsoutputs/D.suzukii_nameuro.png", units="in", width=10, height=5, res=700)
layout(matrix(c(1, 2, 3, 3), nrow = 2, byrow = TRUE), heights = c(0.80, 0.20)) # 2 plots on top, 1 legend area below
...
knitr::include_graphics("../dsoutputs/D.suzukii_nameuro.png")
```
