...
```{r}
png("../dsoutputs/D.suzukii_aus_sm.png", units="in", width=4, height=4, res=700)
layout(matrix(c(1, 2), nrow = 2, byrow = TRUE), heights = c(0.8, 0.2)) # 2 plots on top, 1 legend area below
...
### Host categorisation risk
- Cate
```{r}
png("../dsoutputs/hosteis.png", units="in", width=8, height=8, res=700)
ei_vals_all <- values(aus_ei, na.rm = TRUE) # All EI values across Australia, regardless of host presence
ei_df_all   <- data.frame(EI = as.vector(ei_vals_all))

ei_host_mask   <- mask(aus_ei, hosts_ei_aligned) # EI values restricted to cells where any host commodity is present
...
p1 <- ggplot() +
  ...
  labs(title = "EI across Australia, with host density by EI values",
       x = "Ecoclimatic Index (EI)", y = "Density") +
  theme_minimal() +
  theme(legend.position = "bottom")

ei_vals   <- values(aus_ei, na.rm = FALSE) # Pull EI and host code with NA-preserving alignment so vectors match cell-for-cell
...
combo <- combo %>% left_join(hostcsv, by = c("host_code" = "Value")) # join against lookup table for readable commodity labels
...
p2 <- ggplot(combo, aes(x = EI, fill = host_type)) +
...
p1 / p2 # combine as a multi panel figure
invisible(dev.off())
